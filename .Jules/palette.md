...
**Learning:** Making the close button of a modal difficult to click by having it evade the cursor forces users into an unexpected precision mini-game right when they expect to easily dismiss a popup.
**Action:** Implemented a close button that teleports to a random position within the modal up to 5 times upon cursor hover before finally allowing a click.
