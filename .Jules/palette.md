...
**Learning:** Limiting the sheer amount of interaction a user can have by adding a "stamina" mechanic to their cursor makes simple navigation incredibly tedious, forcing them to pause frequently.
**Action:** Implemented a depleting "Cursor Stamina" bar that tracks mouse movement distance. If the user moves the mouse too much and exhausts the stamina, a full-screen "Cursor Exhausted" overlay appears, blocking all interaction until the stamina slowly recharges.
