...
**Learning:** Making an interface intentionally frustrating (but technically usable) takes careful tuning to ensure the user doesn't just give up, but rather invests a disproportionate amount of time for a minor reward.
**Action:** Implemented a 'manual loading bar' for opening modals, requiring rapid clicking to combat a constant progress drain.
## 2024-10-31 - Evasive Close Buttons
**Learning:** Found an amazing opportunity to enforce user patience. If they want to close a modal, they should have to prove their intent by chasing the close button around the screen, creating a frustrating but technically usable interaction.
**Action:** Implemented evasive close buttons. The button tracks mouseover events and rapidly changes its position absolutely within the modal to dodge the cursor. It only allows itself to be clicked after successfully evading the user 5 times.
